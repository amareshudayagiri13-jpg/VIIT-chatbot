  ...
  return (
    <>
      <style>{`
        ...
        .qr-btn { transition: opacity 0.2s ease; }
      ...
      <div className="flex flex-col" style={{ height: "100dvh", background: "#e8f0fe", overflow: "hidden" }}>
        ...
        <div className="shrink-0 flex items-center gap-3 shadow-md"
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className="logo-wrap rounded-full overflow-hidden" style={{ width: 44, height: 44, padding: 2 }}>
              <img src={LOGO} alt="Vignan" className="w-full h-full object-cover rounded-full" />
            </div>
            <span className="font-extrabold text-white tracking-widest" style={{ fontSize: 8, marginTop: 2 }}>VIGNAN</span>
          </div>
            ...
            <div key={i} className={`flex items-end ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
              {msg.type === "bot" && (
                <div className="flex flex-col items-center shrink-0 mr-2">
                  <div className="rounded-full overflow-hidden shadow"
                    style={{ width: 28, height: 28, background: "white", border: "2px solid #e0e0e0", padding: 1 }}>
              ...
              <div
                className={`rounded-2xl shadow-sm leading-relaxed text-sm sm:text-base ${
                  msg.type === "user" ? "text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border border-blue-100"
                }`}
                style={{
                  maxWidth: "min(75%, 480px)",
                  padding: "10px 14px",
                  background: msg.type === "user" ? "linear-gradient(135deg,#1565C0,#42A5F5)" : undefined,
                }}
              >
                {msg.isTyping ? (
              ...
              {msg.type === "user" && (
                <div className="rounded-full shrink-0 flex items-center justify-center text-xs font-bold text-white shadow ml-2"
                  style={{ width: 32, height: 32, background: "#42A5F5" }}>U</div>
              )}
            </div>
          ))}
          <div ref={bottomRef} />
        ...
        {/* Quick Replies */}
        <div className="shrink-0 bg-white border-t border-blue-100 px-3" style={{ paddingTop: 10, paddingBottom: 14 }}>
          <p className="text-xs text-gray-400 font-medium mb-2">Quick Replies:</p>
          <div className="no-scrollbar flex gap-2 overflow-x-auto" style={{ paddingBottom: 4, paddingTop: 2 }}>
            {quickReplies.map((btn) => (
              <button key={btn} onClick={() => handleQuickReply(btn)}
                className="qr-btn shrink-0 text-white font-semibold rounded-full shadow text-xs sm:text-sm"
                style={{ background: "linear-gradient(135deg,#1565C0,#42A5F5)", padding: "7px 16px", whiteSpace: "nowrap" }}>
                {btn}
              </button>
            ))}
          </div>
        ...
        <div className="shrink-0 bg-white border-t border-blue-100 flex items-center gap-2 px-3 py-2 shadow-md">
          <input value={input} onChange={(e) => setInput(e.target.value)}
            ...
            className="flex-1 border-2 border-blue-200 rounded-full outline-none focus:border-blue-400 transition text-sm sm:text-base"
